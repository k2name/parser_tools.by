...
stats = {'products': 0, 'new': 0, 'updated': 0, 'deleted': 0, 'skipped': 0, 'error': 0}
...
def parse_large_xml(file_path):
    ...
    for event, elem in context:
        ...
        if event == "end" and elem.tag == "offer":
            ...
            # создаем очищенный продукт
            cleared_product = {}
            for param in current_product:
                try:
                    if param in params:
                        cleared_product[param] = current_product[param]
                except:
                    pass
            current_product.clear()

            # Сохраняем данные о текущем товаре
            if cleared_product["okdp"] != None:
                products.append(cleared_product)
...
def parse_filedata(products_from_site):
    ...
    for product in products_from_site:
        ...
        if product['okdp'] not in products:
            products[product['okdp']] = product
            products[product['okdp']]['category_id'] = tmp
            del products[product['okdp']]['parentid1']
            del products[product['okdp']]['parentid_name1']
            del products[product['okdp']]['parentid2']
            del products[product['okdp']]['parentid_name2']
            del products[product['okdp']]['parentid3']
            del products[product['okdp']]['parentid_name3']
            del products[product['okdp']]['parentid4']
            del products[product['okdp']]['parentid_name4']
...
def compare_categories(categories_from_file, categories_from_db):
    ...
    for cat_id in categories_from_db:
        if (cat_id not in categories_from_file) and (categories_from_db[cat_id]['status'] != 'hidden_final'):
    ...
    for cat_id in categories_from_file:
        ...
        elif any(categories_from_file[cat_id][key] == categories_from_db[cat_id][key] for key in keys_to_compare) and categories_from_db[cat_id]['status'] == 'hidden_final':
...
def compare_products(products_from_file, products_from_db):
    ...
    stats['products'] = len(products_from_file)

    compare_idents = []
    # Сравниваем продукты
    for product_okdp in products_from_file:
        if product_okdp not in products_from_db:
                result = db.insert_products(products_from_file[product_okdp], global_timestamp)
                if result:
                    #print(f'sqlite: Добавлен продукт: {products_from_file[product_okdp]["name"]}')
                    stats['new'] += 1
                else:
                    stats['error'] += 1
                    print(f'sqlite: Ошибка при добавлении продукта')
                    print(products_from_file[product_okdp])
        else:
            pr_file = products_from_file[product_okdp]
            pr_db = products_from_db[product_okdp]
            dict2compare = ['barcode', 'brand', 'country', 'description', 'media_img', 'name', 'price', 'prop_importer', 'prop_manufacturer', 'prop_purpose']
...
def compare_wp_categories():
    ...
    # Собираем категории
    db_categories = rows_to_dict(rows=db.get_categories(), key_column='id')
    categories = {}
...
def compare_wp_products():
    ...
    many_id_2del = []
    print('Сравниваем продукты...')

    # собираем товары из БД
    db_products = rows_to_dict(rows=db.get_all_products(), key_column='okdp')
    max_len = len(db_products)
    cur_len = 0
    for okdp in db_products:
        cur_len += 1
        # проверяем статус и реагируем на него
        status = db_products[okdp]['status']
        if status == 'new':
            print(f"{cur_len}/{max_len} Добавляем новый продукт: {db_products[okdp]['name']}")
            product_json = product_generator(db_products[okdp])
            status_code, result = wp.create_product(product_json)
            if status_code is not None and status_code == 201:
                wp_id = int(result['id'])
                db.update_product_wpid(okdp, wp_id)
        elif status == 'updated':
            print(f"{cur_len}/{max_len} Обновляем продукт: {db_products[okdp]['name']}")
            product_json = product_generator(db_products[okdp])
            status_code, result = wp.update_product(db_products[okdp]['wp_id'], product_json)
            if status_code is not None and status_code == 201:
                wp_id = result['id']
                db.update_product_wpid(okdp, wp_id)
        else:
        ...
        if db_products[okdp]['timedata'] != global_timestamp:
            result = db.delete_product(okdp)
            if db_products[okdp]['wp_id'] != None:
                print(f"{cur_len}/{max_len}  Удаляем продукт: {db_products[okdp]['name']}")
                many_id_2del.append(db_products[okdp]['wp_id'])
    ...
    if wp_products is None:
        ...
        if len(many_id_2del) > 0:
            print(f"Найдено {len(many_id_2del)} продуктов для удаления.")
            i = 0
            chunk_size = 50
            for chunk in get_chunks(many_id_2del, chunk_size):
                i += 1
                response = wp.batch_delete_product(chunk)
                if response:
                    print(f"Удалено продуктов mysql: {i*chunk_size}/{len(many_id_2del)}")
                else:
